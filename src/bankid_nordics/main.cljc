(ns bankid-nordics.main
  "Kotodama WASM entrypoint for the Bankid_nordics clean-room actor (L4) — Clojure port.
  ...
  facts (`bankid_nordics.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "bankid_nordics")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"bankid_nordics.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "bankid_nordics-compat" :tier tier :entities entities} 200])
...
(defn start! [] :bankid_nordics-compat/ready)
